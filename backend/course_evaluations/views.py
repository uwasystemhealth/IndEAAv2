...
class CourseEvaluationGenerateReport(viewsets.ReadOnlyModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        filtered_queryset = self.queryset.get(pk=self.kwargs["course_evaluation_id"], coordinators=request.user)
        print(filtered_queryset)
        # serializer = CourseEvaluationDetailSerializer(filtered_queryset)
        # # return Response(serializer.data)

        # instance = self.get_object()
        # file_handle = open(f"{STORAGE_DIRECTORY}/{self.id}")
